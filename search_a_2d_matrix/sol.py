class Solution:
    ...
    def searchMatrix(self, matrix, target):
        ...
        if target < matrix[0][0] or target > matrix[m-1][n-1]:
            return False
        r = self.bRow(matrix, target, 0, m-1)
        if r != -1:
            return self.bCol(matrix, r, target, 0, n-1)
        return False
    def bRow(self, matrix, t, s, e):
        if s > e:
            return -1
        if s == e:
            return s
        m = s + (e-s)/2
        if matrix[m][0] <= t and t <= matrix[m][-1]:
            return m
        elif matrix[m][-1] < t:
            return self.bRow(matrix, t, m+1, e)
        else:
            return self.bRow(matrix, t, s, m-1)
    def bCol(self, matrix, row, t, s, e):
        if s > e:
            return False
        if s == e:
            if matrix[row][s] == t:
